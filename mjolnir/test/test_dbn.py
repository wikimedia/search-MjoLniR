...
def test_dbn_train(hive_context):
    ...
    test = sorted([row for row in data if row.norm_query == u'test'], key=lambda row: row.relevance, reverse=True)
    assert test[0].hit_page_id == 1111
    assert test[3].hit_page_id == 3333

    zomg = sorted([row for row in data if row.norm_query == u'zomg'], key=lambda row: row.relevance, reverse=True)
