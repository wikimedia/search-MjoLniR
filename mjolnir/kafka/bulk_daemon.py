...
CONFIG = {
    ...
    'search_popularity_score': lambda client_for_index, message: ImportExistingIndices(  # type: ignore
        client_for_index, message,
        chunk_size=100,
        # Increased to 3 min from default of 10s. Things don't typically take
        # this long, but sometimes they do and the whole import fails when it
        # occurs.  Latency can be checked via logs generated by elasticsearch.
        request_timeout=180
    ).run(),
...
def bulk_import(**kwargs) -> Tuple[int, int, int]:
    ...
    for ok, result in streaming_bulk(raise_on_exception=False, raise_on_error=False, **kwargs):
