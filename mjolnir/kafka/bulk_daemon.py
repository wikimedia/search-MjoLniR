...
def _decode_response_as_text_lines(file_uri: str, res: Response) -> Iterator[str]:
    ...
    Downloads remote files to disk, rather than streaming directly, to support
    compressed files. We experimented with streamed decompression using
    GzipFile but it wasn't as robust as downloading the files up front.
    """
    ...
    if suffix.lower() == '.gz':
        ...
        if res.encoding is None:
            res.encoding = 'utf-8'
        # This will decode utf8 bytes to str AND strip trailing \n
